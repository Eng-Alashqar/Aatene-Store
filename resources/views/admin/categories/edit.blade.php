    ...
    <div class="app-main flex-column flex-row-fluid" id="kt_app_main">
        ...
        <div class="d-flex flex-column flex-column-fluid">
            ...
            <x-elements.toolbar lable="ادارة الأقسام" back_url="admin.categories.index" previews="قائمة الأقسام"
                current="تعديل قسم  {{ $category->name }}" />
            <!--end::Toolbar-->
            <!--begin::Content-->
            <div id="kt_app_content" class="app-content flex-column-fluid">
                ...
                <div id="kt_app_content_container" class="app-container container-xxl">
                    {{-- <x-error-show-list /> --}}
                    <form class="form d-flex flex-column flex-lg-row" action="{{ route('admin.categories.update',$category->id) }}"
                            ...
                            <div class="card card-flush py-4 ">
                                ...
                                <div class="card-header border-0 ">
                                    <h3 class="card-title align-items-start flex-column">
                                        <span class="card-label fw-bold fs-3 mb-1"> تعديل قسم {{ $category->name }}</span>
                                    </h3>
                                </div>
                                <!--end::Header-->
                                <!--begin::Body-->
                                <div class="card-body py-3">
                                    <div class="mb-5">
                                        <x-form.input-with-lable lable="اسم القسم" name="name"
                                            placeholder="ادخل اسم للقسم" :value="$category->name" />
                                    </div>
                                    <div class="mb-5">
                                        <x-form.textarea-with-lable lable="وصف القسم" name="description"
                                            placeholder="ادخل وصف للقسم"  :value="$category->description" />
                                    </div>
                                    <div class="mb-5">
                                                ...
                                                <optgroup label="{{ $parent->name }}">
                                                    ...
                                                    @foreach ($parent->children as $children)
                                                        <option value="{{ $children->id }}"
                                                            @selected($category->parent_id == $children->id)>
                                    ...
                                    <div class="mb-5">
                                        ...
                                        <select data-control="select2" name="status" @class([
                                            ...
                                            <option value="active"  @selected($category->status == 'active')>قسم نشط</option>
                                            <option value="archive" @selected($category->status == 'archive')> قسم مؤرشف
                                            </option>
                                        </select>
                                    </div>
                                </div>
